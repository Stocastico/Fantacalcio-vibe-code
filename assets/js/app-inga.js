...
;(function (global) {
     ...
     * spostamenti di crediti fra un giocatore e l'altro. Sono il piano di chi ti
     * ha passato la lista, e chi chiama non ne ha bisogno: gli basta sapere se
     * può rilanciare e fino a quanto.
     *
     * Parte accesa a ogni apertura della pagina, apposta: se il file passa di
     * mano, passa già chiuso. Non è una cassaforte — chi apre il sorgente del
     * file i numeri li trova — è per non averli sotto gli occhi di tutti.
     */
    let riservato = true;
    ...
    function renderPurchases() {
        ...
        for (const p of engine.purchases) {
            ...
            if (p.max !== null && p.max !== undefined && p.price > p.max) {
                ...
                price.title = riservato ? 'Sopra il tetto previsto per lui' : `Sopra il tetto di ${p.max}`;
            }
    ...
    function renderBanner() {
        ...
        if (riservato) {
            banner.textContent = diff > 0
                ...
                : 'ℹ️ Restano crediti non assegnati a nessuno in lista.';
            return;
        }
        banner.textContent = diff > 0
    ...
    function importaCSV(testo, nomeFile) {
        ...
        const parti = [riservato
            ? `✅ Caricati ${res.count} giocatori${nomeFile ? ` da ${nomeFile}` : ''}.`
            : `✅ Caricati ${res.count} giocatori${nomeFile ? ` da ${nomeFile}` : ''}, somma dei tetti ${engine.sumMax()}.`];
    ...
    function calculateBid() {
        ...
        if (advice.status === 'over') {
            say(out, riservato
                ? `⚠️ Puoi rilanciare a ${advice.bid}, ma è già sopra il piano previsto per lui: quei crediti li togli agli altri della lista. Il limite vero è ${advice.cap}.`
                : `⚠️ Puoi rilanciare a ${advice.bid}, ma sono ${advice.overBy} crediti sopra il tetto di ${advice.max}: quei crediti li togli agli altri giocatori della lista. Il limite vero è ${advice.cap}.`, 'warn');
            return;
        }

        say(out, riservato
            ? `💰 Rilancia a ${advice.bid}: sei ancora dentro il piano (il limite di spesa è ${advice.cap}).`
            : `💰 Rilancia a ${advice.bid} (il tetto per lui è ${advice.max}, il limite di spesa ${advice.cap}).`, 'ok');
    }
    ...
    function confirmWin() {
        ...
        if (res.over) {
            parti.push(riservato
                ? 'È sopra il piano previsto per lui: i crediti li recupero dagli altri.'
                : `Sono ${res.price - res.player.max} sopra il tetto di ${res.player.max}: tolti agli altri.`);
        }
        if (res.redistribution.changes.length) parti.push(descriviRidistribuzione(res.redistribution));
        if (res.unabsorbed) {
            parti.push(res.unabsorbed > 0
                ? (riservato ? 'Restano crediti non assegnati: la lista è corta.' : `${res.unabsorbed} crediti non assegnati: la lista è corta.`)
    ...
    $('btnRivela').addEventListener('click', () => {
        if (riservato && !confirm(
            'Stai per mettere a schermo i tetti di spesa e gli spostamenti di crediti.\n\n' +
            'Sono il piano di chi ti ha passato la lista: falli vedere solo se non c\'è nessuno che guarda.\n\n' +
            'Procedo?')) return;
        ...
        for (const box of ['outCheck', 'outRandom', 'outAuction', 'outBuy', 'outSettings', 'outImport', 'outSave']) say($(box), '');
        render();
    ...
    $('btnUndo').addEventListener('click', () => {
        ...
        say($('outBuy'), riservato
            ? `↩️ Annullato: ${label}. Rimesso in lista col suo tetto.`
            : `↩️ Annullato: ${label}. Rimesso in lista con tetto ${p.max}.`, 'ok');
        closeAuction();
        render();
    });
